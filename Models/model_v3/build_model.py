...
class Model(nn.Module):
    ...
    def forward(self, x):
        x_1 = self.CNN_3x3(x)
        x_2 = self.CNN_5x5(x)
        concat = torch.cat([x_1, x_2], 1)
        age = self.AgeNN(concat)
        gender = self.GenderNN(concat)
        race = self.RaceNN(concat)
        return age, gender, race
    
if __name__ == '__main__':
    model = Model()
    torch.save(model, '.\\Models\\model_v3\\model_v3.pt')

    info_path = '.\\Models\\model_v3\\model_v3_info.txt'
    f = open(info_path, 'w')
